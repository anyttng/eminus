...
public final class CompositePass implements AutoCloseable {
    ...
    private static final int SIZE = new Std140SizeCalculator().putMat4f().putFloat().get();
    ...
    public void draw(FarTarget far, RenderTarget game, Matrix4fc farViewProjection, Matrix4fc gameViewProjection) {
        RenderSystem.assertOnRenderThread();
        write(gameViewProjection, farViewProjection);
    ...
    private void write(Matrix4fc gameViewProjection, Matrix4fc farViewProjection) {
        reproject.set(farViewProjection).invert();
        reproject.mulLocal(gameViewProjection);
